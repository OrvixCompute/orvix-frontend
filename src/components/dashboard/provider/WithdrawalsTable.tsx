...
export function WithdrawalsTable() {
  ...
  return (
    <>
      {/* Stacked below sm: four mono columns need more width than a phone has,
          and a table there hides its last columns behind an invisible scroll. */}
      <div className="space-y-2 sm:hidden">
        {withdrawals.map((w) => (
          <Card key={w.id} className="space-y-2 p-3">
            <div className="flex items-baseline justify-between gap-3">
              <span className="font-mono text-sm text-text-primary">
                {formatUsdcAmount(w.amount)} USDC
              </span>
              <Badge className={cn(STATUS_CLASS[w.status] ?? "text-text-muted")}>{w.status}</Badge>
            </div>
            <div className="flex flex-wrap items-baseline gap-x-3 font-mono text-[11px] text-text-tertiary">
              <span>{formatDateTime(w.queued_at)}</span>
              ...
              {w.metadata?.manual_approval_required && <span>manual review</span>}
            </div>
            {w.status === "failed" && (
